...
model_manual_LacZ <- lm(time_half_max ~ log10(max_expression), data = sf_prep %>% filter(automated == 'Manual'))

manual_LacZ_m <- round(coef(model_manual_LacZ)[2],2)
manual_LacZ_b <- round(coef(model_manual_LacZ)[1],8)
manual_LacZ_R2 <- round(summary(model_manual_LacZ)$r.squared,2)

model_automated_LacZ <- lm(time_half_max ~ log10(max_expression), data = sf_prep %>% filter(automated == 'Automated'))

automated_LacZ_m <- round(coef(model_automated_LacZ)[2],2)
automated_LacZ_b <- round(coef(model_automated_LacZ)[1],8)
automated_LacZ_R2 <- round(summary(model_automated_LacZ)$r.squared,2)

p_model_manual_LacZ <- lm(time_half_max ~ log10(max_expression), data = sf_prep %>% filter(automated == 'Manual'))

p_manual_LacZ_A <- round(coef(lm(time_half_max ~ poly(log10(max_expression), 2), data = LZ_prep_for_fitting))[3], 1)
p_automated_LacZ_A
p_manual_LacZ_B
p_automated_LacZ_B
p_manual_LacZ_C
...
LacZ_linear  <- LacZ_base_split + geom_smooth(method = 'lm', se = FALSE, color = 'black') +
...
LacZ_quadratic <- LacZ_base_split + geom_smooth(method = 'lm', formula = y ~ poly(x, 2), se = FALSE, color = 'blue') +
